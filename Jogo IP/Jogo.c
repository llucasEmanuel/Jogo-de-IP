...
Enemy *inicializarInimigos(Window janela);
...
void criarFase(Player *jogador, Window janela) {
    ...
    Enemy *inimigos = inicializarInimigos(janela);
...
Enemy *inicializarInimigos(Window janela) {
    ...
    inimigos[0].coordenadas = (Vector2) {(float) (rand() % (janela.width - (inimigos[0].textura.width * 2))), (float) (rand() % (janela.height - (inimigos[0].textura.height * 2)))};
